...
public class JraVanClient : IDisposable
{
    ...
    public IEnumerable<T> GetRecords<T>(string dataSpec, string key, int option) where T : new()
    {
        ...
        try
        {
            ...
            while (true)
            {
                ...
                // レコードIDによるフィルタリング
                if (!ShouldProcessRecord(recordBytes, targetRecordId, encoding))
                {
                    // 対象外のレコードなのでスキップ
    ...
    /// レコードが処理対象かどうかを判定します。
    /// </summary>
    private bool ShouldProcessRecord(byte[] recordBytes, string? targetRecordId, Encoding encoding)
    {
        if (string.IsNullOrEmpty(targetRecordId)) return true;
        if (recordBytes.Length < 2) return false;
